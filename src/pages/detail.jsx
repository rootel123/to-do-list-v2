  ...
  return (
    <div className="min-h-screen flex justify-center items-center">
      ...
      <span className="ml-4">
        {isEdit ? (
          <form className="flex" onSubmit={onSubmitEdit}>
            <input
              className="border-2 focus:outline-none focus:border-blue-300 mr-3 px-2 py-1 rounded-md"
              type="text"
              value={editTodo}
      ...
      <button
        ...
        className="bg-green-400 hover:bg-green-600 active:bg-green-400 rounded-md px-2 h-8 flex justify-center items-center ml-4"
      >
        <FiCheckSquare /> {isDone === "true" ? "완료취소" : "완료"}
      </button>
      <button
        onClick={onClickEditToggle}
        className="bg-blue-400 hover:bg-blue-600 active:bg-blue-400 rounded-md px-2 h-8 flex justify-center items-center ml-4"
      >
        <FiEdit /> {isEdit ? "취소" : "편집"}
      </button>
      <button
        onClick={onClickIsDel}
        className="bg-red-400 hover:bg-red-600 active:bg-red-400 rounded-md px-2 h-8 flex justify-center items-center ml-4"
      >
        <RiDeleteBin6Line />
        삭제
      </button>
